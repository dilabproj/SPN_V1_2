...
def execute(config, training_data, training_label, training_length, testing_data, testing_label, testing_length, 
    ...
    for epoch in range(config.epoch_size):
        ...
        for idx in tqdm(range(len_t)):
            ...
            for sample in training_data[idx*batch_size:idx*batch_size+batch_size]:
                input_row.append(torch.tensor(sample[:30000,:]))   
            
            input_row = torch.nn.utils.rnn.pad_sequence(input_row, batch_first=True).float().cuda()
            
            print(input_row.shape)
            
            #input_row = torch.as_tensor( training_data[idx*batch_size:idx*batch_size+batch_size]).float().cuda()

            input_label = torch.as_tensor(
                training_label[idx*batch_size:idx*batch_size+batch_size]).cuda()

            input_length = training_length[idx * batch_size : idx*batch_size+batch_size]
        ...
        for idx in tqdm(range(len_t)):
            ...
            for sample in testing_data[idx*batch_size:idx*batch_size+batch_size]:
                input_row.append(torch.tensor(sample[:30000,:]))   
            
            input_row = torch.nn.utils.rnn.pad_sequence(input_row, batch_first=True).float().cuda()
            
            #input_row = torch.as_tensor( testing_data[idx*batch_size:idx*batch_size+batch_size]).float().cuda()
            ...
            earliness = cuil.update_earliness_eval(t_stops,
                                              earliness,
                                              input_length,
                                              config.total_length,
                                              config.series_ratio)
        ...
        if(epoch%10 ==0):
            wandb.save(model_path+"/"+str(config.alpha)+"_"+str(epoch)+"model-var-cont.pt")
            torch.save(result_model.state_dict(), model_path+"/"+str(config.alpha)+"_"+str(epoch)+"model-var-cont.pt")
...
if __name__ == "__main__":
    ...
    for seed in range(5, 6):
        ...
        config = Config(message="20210527-og-kfold-save-cont", batch_size=16, 
                        total_length=30000, dataset_name="ICBEB",
                        series_ratio=1, alpha = 0.5, output_size=9,
        ...
        input_data = data #uio.resize(data, config.total_length, config.series_ratio)
        ...
        if (config.pretrained):
            ...
            pretrained = model_path+"/0.5model-var.pt"
        else:
        ...
        if (config.pretrained):
            wandb.save(model_path+"/"+str(config.alpha)+"model-var-cont.pt")
            torch.save(result_model.state_dict(), model_path+"/"+str(config.alpha)+"model-var-cont.pt")
        else:
            wandb.save(model_path+"/"+str(config.alpha)+"model-var.pt")
            torch.save(result_model.state_dict(), model_path+"/"+str(config.alpha)+"model-var.pt")
